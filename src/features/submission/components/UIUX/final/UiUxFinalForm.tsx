...
export default function UiUxFinalForm({ teamId }: UiUxFinalFormProps) {
  const router = useRouter();
  const methods = useForm<UiUxFinalSubmissionData>();
  ...
  const onSubmit = async (data: UiUxFinalSubmissionData) => {
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="relative flex flex-col w-full md:w-[80%] gap-6">
             ...
             <div className="w-full">
                ...
                <UploadButton
                    ...
                    onSuccess={(url) => setValue("ppt_url", url, { shouldValidate: true })}
                >
                    {({ open, isLoading }) => (
                    <button
                        type="button"
                ...
                {/* Validation message for PPT if needed, though UploadButton usually handles it visually. We can add a required rule to hidden input if strict */}
                 <input type="hidden" {...register("ppt_url", { required: "PPT is required" })} />
                 {errors.ppt_url && <p className="text-red-500 text-xs mt-1">{errors.ppt_url.message}</p>}
             ...
             <div className="w-full">
                <FormInput
                    id="video_demo_url"
                    label="link google drive video_demo"
